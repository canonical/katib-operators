...
This document outlines the processes and practices recommended for contributing enhancements to `katib-ui`.
...
You can use the environments created by `tox` for development. For example, to load the `fmt` environment into your shell, run:

```shell
tox --notest -e fmt
source .tox/fmt/bin/activate
```
...
# Deploy the charm
juju deploy ./katib-ui_ubuntu-20.04-amd64.charm \
    --resource oci-image=$(yq '.containers."katib-ui".resources."oci-image"."upstream-source"' metadata.yaml)
